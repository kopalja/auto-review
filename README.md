...
**Checks PRs in monitored GitHub repositories and generates two independent reviews, one from Codex and one from Claude.**

Python standard-library runner for this Raspberry Pi. Uses `gh` for GitHub operations, the existing Codex ChatGPT login for Astra reviews, and the existing Claude Code subscription login for Claude Opus 5.5 reviews. Both reviewers get the same prompt, schema and review input, and run in parallel. PR heads are handled one at a time under a nonblocking file lock.
...
Claude always uses `claude-opus-5-5`. It has its own reasoning level, passed as `--effort`:

```json
"claude_reasoning_effort": "high"
```

Allowed values: `low`, `medium`, `high` (default), `xhigh`, `max`.

`reviewers` selects which reviewers run (default: both):

```json
"reviewers": ["codex", "claude"]
```

Each reviewer has its own state row, retries and comment, so a failure, timeout or limit in one never blocks the other. A reviewer only joins PR heads first seen while it is enabled: enabling Claude does not review heads that already have a Codex review. Disabling a reviewer stops its pending work.
...
- One summary issue comment per reviewer per PR head SHA (headed "Generated by Codex" or "Generated by Claude"), under the authenticated GitHub account. No approvals, merges, or automatic code changes.
...
- Defaults: 3 PR heads per invocation (each gets both reviews in parallel), 15 minutes per model call, 3 attempts per generation/publication stage. Increasing retry delays; GitHub rate-limit responses defer longer.
...
Claude needs an existing **Claude Code subscription login** at `$CLAUDE_CONFIG_DIR/.credentials.json` or `~/.claude/.credentials.json` (created by `claude` → `/login`). The same rules apply: the worker uses an ephemeral copy, never uses an API key, and saves refreshed tokens back only if the host file has not changed. With the default `reviewers`, both logins are required; the runner fails closed if either is missing.
...
Dry runs use **separate state under `var/dry-run/`**, so they cannot change the production baseline or publication queue. Successful output is printed and retained in SQLite; rerunning a dry run reuses cached output. Dry runs consume Codex and Claude usage.
...
- Each reviewer gets its own Bubblewrap worker. It sees a fresh home, the system runtime, the trusted worker/schema, its own binary, and a copy of its own login only. Host home, GitHub credentials, Git caches, other automations, and host processes are absent.
- All network interfaces except private loopback are absent. A Unix-socket relay permits HTTPS CONNECT on port 443 only to `chatgpt.com`, `auth.openai.com` and `api.openai.com` for Codex, and only to `api.anthropic.com`, `platform.claude.com` and `console.anthropic.com` (OAuth refresh) for Claude. Public destination IPs are validated. Other destinations, including home-network addresses, are denied. TLS remains end-to-end.
- Codex shell execution, apps, hooks, browser/computer tools, remote plugins, and user/project instruction loading are disabled. Claude runs with no tools, no MCP servers, no skills, no settings files, no session persistence, and with nonessential traffic (telemetry, updates, error reporting) disabled. PR files are never checked out into the worker.
...
Tests mock GitHub, Codex and Claude calls; they use no network or model credits. `--check` additionally probes real OS isolation. Model availability is established by a successful dry run, not assumed from a model name.
